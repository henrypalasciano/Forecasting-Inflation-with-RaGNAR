...
┣ 📜 quarterly.py                    # Computes RMSEs across 100 RaGNAR runs for the case where the network is chosen on a quarterly basis and stores them in automatically created folders.
┣ 📜 directed_graphs.py              # Computes RMSEs across 100 RaGNAR runs with directed graphs and stores them in automatically created folders.
...
The scripts in the `article_figures_and_tables` folder reproduce the figures and tables from the article. For figures based on a single run, the outputs provided in the `results` folder are used. If you'd like to generate new results using `parallel_comp.py`, you'll need to update the file paths in the relevant scripts accordingly. Note that `parallel_comp.py` does **not** save the rolling mean squared error DataFrames or the randomly generated adjacency matrices by default, as these occupy significant memory. Figures 3 and 8 require this data. To reproduce them using new runs (rather than the data in the `results` folder), you’ll need to modify `parallel_comp.py` to save the necessary outputs. The functions used to reproduce the results in Table 12 of the appendix are located in `directed_graphs.py` and `table_12.py`. Note that `directed_graphs.py` can take a significant amount of time to run. Table 13 can be reproduced by running `quarterly.py` followed by `table_13.py`.  
