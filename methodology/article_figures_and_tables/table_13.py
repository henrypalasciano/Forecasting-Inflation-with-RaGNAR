...
for model in ["global", "standard", "local"]:
    ...
    rmse_0 = pd.read_csv(f"../quarterly_results/rmse_df_{model}_bic_{n}_0.csv", index_col=0)
    rmse_avg_df = rmse_0.copy()
    rmse_avg_df_sq = rmse_0 ** 2
    for i in range(1, 100):
        rmse_i = pd.read_csv(f"../quarterly_results/rmse_df_{model}_bic_{n}_{i}.csv", index_col=0)
        rmse_avg_df += rmse_i
